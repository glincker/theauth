...
| `@glinr/theauth-fastify` | Fastify | `app.register(kavachFastify(kavach), { prefix: '/api/kavach' })` | [Fastify](../guides/frameworks/fastify.md) |
| `@glinr/theauth-nestjs` | NestJS | `KavachModule.forRoot({ kavach })` in `AppModule` | [NestJS](../guides/frameworks/nestjs.md) |
...
|---|---|---|
| `@glinr/theauth-react` | Browser | React hooks and `KavachProvider` |
...
| Package | Description |
|---|---|
| `@glinr/theauth/telemetry` (built-in) | OpenTelemetry hooks via `TelemetryConfig` in `createKavach` |
