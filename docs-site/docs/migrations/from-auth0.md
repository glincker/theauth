...
| Auth0 | TheAuth |
|---|---|
| Tenant | A `createKavach` instance. One per deployment. |
...
| Rule | Deprecated in Auth0 as well. Replace with a lifecycle hook on `createKavach` or a policy. |
...
| Custom domain | Set `baseUrl` and the cookie config on `createKavach`. |
