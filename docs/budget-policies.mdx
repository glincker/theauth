...
<Info>
  Policies stack. An agent can have a per-agent policy, a per-user policy, and a per-tenant policy all active at once. TheAuth evaluates all of them and returns the first one that is exceeded.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Cost attribution" href="/cost-attribution">
  Aggregate token costs per agent for billing reports.
</Card>
  <Card title="Multi-tenant isolation" href="/multi-tenant">
