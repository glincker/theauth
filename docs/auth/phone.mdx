---
title: Phone number
description: Authenticate users with `phoneAuth()` via SMS one-time codes. Supply an `onSendCode` callback, configure expiry, and TheAuth handles rate limiting.
icon: key
---

The `phoneAuth` plugin registers and authenticates users with a phone number and a one-time code. You supply the SMS delivery function. TheAuth handles code generation, expiry, and rate limiting.

## Setup

```typescript title="lib/kavach.ts"
import { createTheAuth } from '@glinr/theauth';
import { phoneAuth } from '@glinr/theauth/auth'; // [!code highlight]
...
<CardGroup cols={2}>
  ...
  <Card title="Passkey" href="/auth/passkey" icon="fingerprint">
    Device biometric auth as a phone-free alternative.
  </Card>
