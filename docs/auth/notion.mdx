---
title: Notion
description: Authenticate users via Notion OAuth 2.0. Create a public integration, configure OAuth redirect URIs in Notion settings, and wire the `notion` provider.
...
```
https://auth.example.com/auth/oauth/callback/notion
```
...
<Info>
  Notion returns user identity as part of the token exchange response, there is no separate `/me` endpoint. TheAuth caches the token payload internally so no extra network call is made.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="GitLab" href="/auth/gitlab" icon="gitlab">
    Another work and productivity OAuth provider.
  </Card>
