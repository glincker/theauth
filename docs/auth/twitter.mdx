...
iconType: brands
title: Twitter / X
description: Authenticate users with Twitter/X via `createTwitterProvider`. Covers OAuth 2.0 PKCE, Developer Portal configuration, and `users.read` and `tweet.read` scopes.
---
...
import { createTwitterProvider } from 'theauth/auth';
...
<Warning>
  Twitter does not return an email address through the standard OAuth 2.0 flow. TheAuth uses a synthetic non-deliverable address (`username@twitter.invalid`) as a placeholder. Do not treat it as a real email.
</Warning>
