...
iconType: brands
title: Stripe
description: Wire Stripe checkout sessions, billing portals, and subscription webhooks via the `stripe` plugin. No `stripe` npm package needed, calls Stripe's REST API directly.
---
...
import { stripe } from '@glinr/theauth/auth';
...
<CardGroup cols={2}>
  <Card title="Polar payment" href="/auth/polar-payment" icon="credit-card">
    Alternative billing integration using Polar subscriptions and webhooks.
  </Card>
  <Card title="Organizations" href="/auth/organizations" icon="building">
    Manage org membership alongside subscription status.
  </Card>
  <Card title="API keys" href="/auth/api-keys" icon="key">
    Issue scoped API keys for server-to-server billing integrations.
  </Card>
  <Card title="Hooks" href="/hooks" icon="bolt">
    React to subscription changes with lifecycle hooks.
  </Card>
</CardGroup>
