...
iconType: brands
title: Spotify
description: Authenticate users with Spotify via `spotifyProvider`. Configures OAuth 2.0 with `user-read-email` and `user-read-private` scopes and supports scope extension.
---
...
import { spotifyProvider } from '@glinr/theauth/auth'; // [!code highlight]
