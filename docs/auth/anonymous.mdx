...
import { anonymousAuth } from '@glinr/theauth/auth'; // [!code highlight]
...
import { cleanupAnonymousUsers } from '@glinr/theauth/auth';
...
<CardGroup cols={2}>
  <Card title="Email and password" href="/auth/email-password" icon="envelope">
    Full registration flow users can upgrade into from a guest session.
  </Card>
  <Card title="Magic link" href="/auth/magic-link" icon="link">
    Passwordless sign-in that anonymous users can upgrade into.
  </Card>
  ...
  <Card title="Hooks" href="/hooks" icon="bolt">
    Run logic on guest creation and upgrade events.
  </Card>
</CardGroup>
