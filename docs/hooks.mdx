...
Pass a `hooks` object to `createTheAuth`:
...
Returning `{ allow: false }` from `beforeAgentCreate` causes the `kavach.agent.create()` call to throw a `TheAuthError` with the reason you provided.
