...
# answer the 3 prompts, then:
cd my-theauth-app && pnpm install && pnpm db:push && pnpm dev
```
</Columns>
...
<Tabs>
  ...
  <Tab title="yarn">
    ```bash title="terminal"
    yarn create @glinr/theauth-app
    ```
  </Tab>
  <Tab title="bun">
    ```bash title="terminal"
    bun create @glinr/theauth-app
    ```
  </Tab>
</Tabs>
...
<Info>
`createTheAuth` is async when using D1. Workers and Deno both support top-level await, so you can also initialize outside the handler if you use a module worker.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" icon="gear" href="/configuration">
    All `createTheAuth()` options and environment patterns.
  </Card>
</CardGroup>
