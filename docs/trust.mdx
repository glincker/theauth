---
title: Trust scoring
description: TrustScore is a 0 to 100 value from each agent audit log, mapping to five named levels that gate autonomy, approval requirements, and rate limit strictness.
icon: user-shield
---
...
<Info>
  Scores are recomputed on demand. TheAuth does not maintain a background scorer. Call `computeScore` or `computeAll` on a schedule that fits your use case, every request, every few minutes, or nightly.
</Info>
