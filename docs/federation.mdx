...
description: Issue and verify short-lived federation tokens so an AgentIdentity can authenticate at a remote TheAuth instance without re-registration or shared databases.
icon: network-wired
---
...
When two services both run TheAuth, an agent created at Service A can authenticate at Service B without creating a new account. The agent's identity, trust score, permissions, and delegation scope travel with it in a short-lived, signed JWT called a federation token.
...
Each TheAuth instance needs its own signing key.
...
Each TheAuth instance can publish its identity at:
...
<CardGroup cols={2}>
  <Card title="W3C DID identity" href="/did" icon="fingerprint">
    Ed25519-backed agent identities that federation tokens are signed with.
  </Card>
  <Card title="Verifiable credentials" href="/verifiable-credentials" icon="circle-check">
    Embed W3C Verifiable Credentials in federation tokens for offline proof.
  </Card>
  <Card title="Delegation" href="/delegation" icon="link">
    Delegate permissions within a single TheAuth instance.
  </Card>
  <Card title="Multi-tenant" href="/multi-tenant" icon="building">
    Isolate tenants within a single instance instead of federating.
  </Card>
</CardGroup>
