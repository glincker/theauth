...
TheAuth applies rate limits at two layers: auth endpoints get IP-based limits to protect against brute force and credential stuffing, and the permission engine enforces per-agent call limits via the `maxCallsPerHour` constraint.
...
Pass a `rateLimit` block to `createTheAuth` to override the defaults or enable Redis:
...
```typescript
import { createRateLimiter } from '@glinr/theauth';
...
```typescript
import { withRateLimit } from '@glinr/theauth';
