...
import { createCookieSessionManager } from '@glinr/theauth/auth'; // [!code highlight]
...
```typescript title="lib/kavach.ts"
import { createJwtSessionModule } from '@glinr/theauth/auth'; // [!code highlight]
...
import { createSessionFreshnessModule } from '@glinr/theauth'; // [!code highlight]
...
import { generateCsrfToken, validateCsrfToken, validateOrigin } from '@glinr/theauth'; // [!code highlight]
...
<ParamField path="freshAge" type="number" default="300 (5 min)">Maximum session age in seconds before a sensitive operation requires re-authentication. The freshness guard returns 403 SESSION_STALE when the session is older than this.</ParamField>
...
<CardGroup cols={2}>
  ...
  <Card title="Ephemeral sessions" href="/ephemeral-sessions" icon="stopwatch">
