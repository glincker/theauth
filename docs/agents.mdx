...
<ParamField path="token" type="string">Bearer token with a kv_ prefix. Shown once at creation, then hashed. Rotate to replace.</ParamField>
...
<ParamField path="name" type="string">Human-readable label for the agent.</ParamField>
<ParamField path="type" type="'autonomous' | 'delegated' | 'service'">Determines how the agent acquires and uses permissions.</ParamField>
<ParamField path="expiresAt" type="Date" default="undefined (never expires)">Optional expiry. After this time the agent status becomes expired.</ParamField>
...
<CardGroup cols={2}>
  <Card title="Permission engine" href="/permissions">
  ...
  <Card title="Token-based auth with adapters" href="/adapters">
