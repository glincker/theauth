...
description: "Authorize agent actions via resource pattern matching and per-permission constraints: rate limits, time windows, IP allowlists, and human approval gates."
icon: user-shield
---
...
TheAuth ships named templates for common patterns. Import them from `theauth`:

```typescript
import { permissionTemplates, getPermissionTemplate } from 'theauth';
