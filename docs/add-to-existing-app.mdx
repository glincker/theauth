...
No by default. Kavach runs its own migrations on first boot. Set `skipMigrations: true` in `createTheAuth` if you want to run them yourself via `kavach-cli migrate`.
</Accordion>
