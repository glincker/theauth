...
4. Set `database.skipMigrations: true` in `createAuth(...)` if the core TheAuth instance is still used for other features (agents, MCP, etc.), so Drizzle does not conflict with Prisma.
...
<Warning>
  The core TheAuth instance (`createAuth`) still uses Drizzle internally
  ...
  tables, it does not replace `createAuth`.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" href="/configuration" icon="sliders">
    Full createAuth() options including database connection setup.
  </Card>
