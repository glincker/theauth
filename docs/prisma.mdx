...
description: Query every TheAuth table through an existing PrismaClient via @theauth/prisma. Supports Prisma transactions without requiring Drizzle as a second ORM client.
icon: database
---


TheAuth ships a Drizzle-based database layer by default. If your project already uses [Prisma](https://www.prisma.io), the `@theauth/prisma` adapter lets you query every TheAuth table through your existing `PrismaClient`, no Drizzle installation required.
...
```bash
pnpm add @theauth/prisma @prisma/client
```

The adapter has no runtime dependency on `theauth` itself, `@prisma/client` is the only peer dep.
...
Copy the models from `node_modules/@theauth/prisma/src/schema.prisma` into your `prisma/schema.prisma`, or use it as a reference to add only the tables you need.
...
model KavachUser {
  ...
  // ... (see full schema in @theauth/prisma/src/schema.prisma)
...
<Warning>
  ...
  `@theauth/prisma` gives you a Prisma-native way to read and write the same
  tables, it does not replace `createKavach`.
</Warning>

## Reference schema

The full reference schema is in `node_modules/@theauth/prisma/src/schema.prisma`. It includes all 30+ TheAuth models with correct column mappings, indexes, and default values.
