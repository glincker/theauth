---
title: Fastify
description: Register TheAuth auth routes on Fastify with `authFastify(kavach)`. Returns an async plugin for agent identity, delegation, audit, and MCP OAuth 2.1 endpoints.
icon: "/brand-icons/fastify.svg"
---

`authFastify(kavach, options?)` returns an async Fastify plugin. Register it with `fastify.register` and use Fastify's built-in `prefix` option to control the mount path.

## Install

```bash
pnpm add theauth @glinr/@glinr/theauth-fastify fastify
```
...
await app.register(authFastify(kavach, { mcp }), {
  prefix: '/api/kavach',
});
