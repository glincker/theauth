...
description: Mount TheAuth auth routes in TanStack Start with `authTanStack(kavach)`. Returns named handlers for a splat route using Web-standard Request.
icon: "/brand-icons/tanstack.svg"
---

`authTanStack(kavach, options?)` returns named route handlers `{ GET, POST, PATCH, DELETE, OPTIONS }`. Mount them in a splat API route file so all TheAuth paths are handled.

## Install

```bash
pnpm add theauth @glinr/@glinr/theauth-tanstack
```
...
const handlers = authTanStack(kavach, { mcp });
