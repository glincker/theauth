...
The `theauth` package has zero framework dependencies. It operates entirely on the Web platform `Request`/`Response` API. Adapter packages wrap the core and expose framework-idiomatic handlers for authentication, authorization, and MCP OAuth routes.
...
|---|---|---|
| [`@theauth/hono`](/platform/adapters/hono) | Hono | `app.route('/api/kavach', kavachHono(kavach))` |
| [`@theauth/express`](/platform/adapters/express) | Express | `app.use('/api/kavach', kavachExpress(kavach))` |
| [`@theauth/nextjs`](/platform/adapters/nextjs) | Next.js App Router | catch-all route `app/api/kavach/[...kavach]/route.ts` |
| [`@theauth/fastify`](/platform/adapters/fastify) | Fastify | `app.register(kavachFastify(kavach), { prefix: '/api/kavach' })` |
| [`@theauth/nuxt`](/platform/adapters/nuxt) | Nuxt (H3) | catch-all file `server/api/kavach/[...].ts` |
| [`@theauth/sveltekit`](/platform/adapters/sveltekit) | SvelteKit | catch-all route `src/routes/api/kavach/[...path]/+server.ts` |
| [`@theauth/astro`](/platform/adapters/astro) | Astro | catch-all page `src/pages/api/kavach/[...path].ts` |
