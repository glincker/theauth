...
```bash
pnpm add theauth @theauth/hono hono @hono/node-server
```
...
// Mount all TheAuth routes (agents, permissions, audit, MCP OAuth)
...
<CardGroup cols={2}>
  ...
  <Card title="Fastify" href="/adapters/fastify" icon="code">
    Plugin-based Node.js adapter for high-throughput apps.
  </Card>
