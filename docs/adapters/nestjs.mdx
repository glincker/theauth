...
```bash
pnpm add theauth @glinr/@glinr/theauth-nestjs
```
...
If you prefer to mount routes imperatively in `main.ts` rather than importing a module, use `authMiddleware` directly:

```typescript
// main.ts
import { NestFactory } from '@nestjs/core';
import { AppModule } from './app.module.js';
import { authMiddleware } from '@glinr/theauth-nestjs';
import { kavach, mcp } from './lib/kavach.js';
