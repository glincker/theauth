---
title: Astro
description: Mount TheAuth auth routes in an Astro app with `kavachAstro(kavach)`. Returns named handlers for a catch-all API page, with full MCP OAuth 2.1 support.
icon: "/brand-icons/astro.svg"
---

`kavachAstro(kavach, options?)` returns named route handlers `{ GET, POST, PATCH, DELETE, OPTIONS, ALL }`. Mount them in a catch-all API page so all TheAuth paths are handled. Use individual named exports or the `ALL` catch-all handler.
