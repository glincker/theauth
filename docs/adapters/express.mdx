---
title: Express
description: Mount TheAuth auth routes on Express with `kavachExpress(kavach)`. Returns a Router with agent identity, delegation, audit, and MCP OAuth 2.1 endpoints pre-mounted.
icon: "/brand-icons/express.svg"
---

`kavachExpress(kavach, options?)` returns an Express `Router` with all TheAuth routes pre-mounted. Use `app.use` to attach it at your chosen path.

## Install

```bash
pnpm add theauth @theauth/express express
...
<CardGroup cols={2}>
  ...
  <Card title="NestJS" href="/adapters/nestjs" icon="code">
