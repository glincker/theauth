---
title: Nuxt
description: Mount TheAuth auth routes in Nuxt with `theAuthNuxt(kavach)`. Returns an H3 EventHandler for a catch-all server route, covering agent identity and OAuth.
icon: "/brand-icons/nuxt.svg"
---

`theAuthNuxt(kavach, options?)` returns an H3 `EventHandler`. Mount it in a catch-all server route so all TheAuth paths are handled.
...
export default theAuthNuxt(kavach, { mcp });
```
