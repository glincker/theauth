---
title: SolidStart
description: Mount TheAuth auth routes in SolidStart with `kavachSolidStart(kavach)`. Returns named route handlers using Web-standard Request, with no conversion layer needed.
icon: "/brand-icons/solid.svg"
---

`kavachSolidStart(kavach, options?)` returns named route handlers `{ GET, POST, PATCH, DELETE, OPTIONS }`. Mount them in a catch-all API route file so all TheAuth paths are handled.

## Install

```bash
pnpm add theauth @theauth/solidstart
```
