---
title: SolidStart
description: Mount TheAuth auth routes in SolidStart with `authSolidStart(kavach)`. Returns named route handlers using Web-standard Request, with no conversion layer needed.
icon: "/brand-icons/solid.svg"
---

`authSolidStart(kavach, options?)` returns named route handlers `{ GET, POST, PATCH, DELETE, OPTIONS }`. Mount them in a catch-all API route file so all TheAuth paths are handled.

## Install

```bash
pnpm add theauth @glinr/@glinr/theauth-solidstart
```
...
const handlers = authSolidStart(kavach, { mcp });
