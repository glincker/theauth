...
description: Test auth-dependent code without a database using @theauth/test-utils. Provides entity factories, in-memory mock auth server, and per-request user overrides.
icon: code
---


`@theauth/test-utils` provides factories, mock servers, and assertion helpers so you can test auth-dependent code without a real database or network.

## Install

```bash
pnpm add -D @theauth/test-utils
```
...
```ts
import { createMockAuthServer, createMockUser } from '@theauth/test-utils';
...
```ts
import { MOCK_USER_ID_HEADER } from '@theauth/test-utils';
...
<CardGroup cols={2}>
  <Card title="Error codes" href="/errors" icon="triangle-exclamation">
    Error codes the assertions check against in test expectations.
  </Card>
  ...
  <Card title="Agent identity" href="/agents" icon="robot">
    Agent creation and permission checking that test-utils wraps.
  </Card>
  <Card title="Hooks" href="/hooks" icon="bolt">
    Test lifecycle hooks that fire on auth events.
  </Card>
</CardGroup>
