...
import { createLastLoginModule } from '@glinr/theauth/auth';
...
All methods return `Result<T>`, either `{ success: true; data: T }` or `{ success: false; error: TheAuthError }`. No exceptions are thrown.
