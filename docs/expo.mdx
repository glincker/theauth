...
```tsx
import { useSignIn, useUser } from '@glinr/theauth-expo';
...
<TheAuthExpoProvider config={{ basePath: '...', storage: secureStorage }}>
  {children}
</TheAuthExpoProvider>
```
...
| `storage` | `AuthStorage?` | Token persistence adapter. Defaults to in-memory |
