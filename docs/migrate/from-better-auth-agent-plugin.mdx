...
TheAuth has no `emailAndPassword` config key, its built-in password module authenticates by username (see [Username and password](/auth/username)). The rest of the human-auth config (`organization`, `twoFactor`, etc.) maps one-to-one and keeps its plugin shape, imported from `@glinr/theauth/auth` rather than `@glinr/theauth/plugins`.
...
If you were running a separate MCP OAuth server alongside better-auth (because the `@better-auth/mcp` plugin is a thin OIDC wrapper without agent semantics), you can retire it. TheAuth mounts the MCP endpoints on the same adapter:
...
export const { GET, POST, PATCH, DELETE, OPTIONS } = theAuthNextjs(kavach, { mcp });
```
...
Agent rows in better-auth live in whatever table the plugin writes to (check the plugin's schema migration, usually `agents` or `bettterauth_agents`). TheAuth uses `agent_identities` by default.
...
3. TheAuth agent rows remain, they just stop being read. The token hashes are compatible enough that the same bearer keeps working on better-auth until it rotates.
