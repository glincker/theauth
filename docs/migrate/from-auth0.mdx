...
| Custom domain, cookie domain | Set `baseUrl` and the cookie config on `createAuth`. |
...
Wrap the root in `<AuthProvider>`, then the hooks work the same way they do in the Auth0 SDK.
