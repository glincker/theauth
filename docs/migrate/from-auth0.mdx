...
| Application (Regular Web, SPA, Native) | `@theauth/react`, `@theauth/vue`, etc., plus the adapter for your framework. |
...
import { organization, rbac, mcp } from 'theauth/plugins';
...
```tsx
// AFTER (@theauth/react)
import { useUser, useSignOut } from '@theauth/react';
...
- A hosted B2B customer-admin console. TheAuth has an embeddable `@theauth/dashboard` React component for operators, not a hosted SaaS console.
...
A minimal, in-memory version of the AFTER patterns lives at [`examples/migrate-from-auth0`](https://github.com/glincker/theauth/tree/main/examples/migrate-from-auth0). It pulls `theauth` via the monorepo `workspace:*` protocol so the script stays pinned to the same version the docs describe, and the smoke test runs in CI on every push.
