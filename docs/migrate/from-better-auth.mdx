...
import { organization, twoFactor } from 'theauth/plugins';
...
import { createKavachClient } from '@theauth/client';
...
```ts
import { createKavach } from 'theauth';
import { cookieAuth } from 'theauth/adapters';
...
**Is Prisma supported?**
Yes. Install `@theauth/prisma` and pass a `PrismaClient` as the database backend. See the [Prisma adapter docs](/docs/prisma) for the setup.
