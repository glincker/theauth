...
```typescript
import { exportAuditAsVC, listAuditRecords } from '@glinr/theauth/vc';
...
The `@context` array includes both `https://www.w3.org/ns/credentials/v2` and `https://theauth.dev/contexts/audit/v1.jsonld`. The theauth context URL is a stable identifier for this schema, it does not need to resolve at runtime.
