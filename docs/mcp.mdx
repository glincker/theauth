...
<Info>
  The well-known endpoints are served at the root of your domain, not under `basePath`. TheAuth registers them separately so MCP clients can discover your auth server from any path.
</Info>
...
import { createMcpModule } from '@glinr/theauth';
...
```typescript
import { buildUnauthorizedResponse } from '@glinr/theauth';
...
<AccordionGroup>
  <Accordion title="Protected resource metadata document">
    TheAuth generates this document automatically from your `mcp` config and registered servers:
