...
|--------|--------|--------------|
| `emailPassword` | `theauth/auth` | Email + password with verification and reset |
| `passkey` | `theauth/auth` | WebAuthn/FIDO2 biometric auth |
| `magicLink` | `theauth/auth` | Passwordless email links |
| `emailOtp` | `theauth/auth` | One-time password codes via email |
| `twoFactor` | `theauth/auth` | TOTP 2FA with backup codes |
| `multiSession` | `theauth/auth` | Session limits and device management |
| `organization` | `theauth/auth` | Organizations with RBAC |
| `apiKeys` | `theauth/auth` | Static API key management |
| `admin` | `theauth/auth` | User management, banning, impersonation |
| `stripe` | `theauth/auth` | Stripe billing integration |
| `polar` | `theauth/auth` | Polar payment integration |
...
<CardGroup cols={2}>
  ...
  Mount TheAuth on your framework.
</Card>
</CardGroup>
