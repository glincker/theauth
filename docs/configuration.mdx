---
title: Configuration
description: Reference for every `createTheAuth()` option in `TheAuthConfig`. Covers database providers, agent settings, MCP OAuth, session config, and anomaly detection.
icon: gear
---


`createTheAuth()` accepts a `TheAuthConfig` object. The only required field is `database`. Everything else is optional and enables features incrementally.
...
<ParamField path="hooks" type="AuthHooks">Lifecycle hooks for sandboxing and custom validation.</ParamField>
...
|--------|--------|--------------|
| `emailPassword` | `@glinr/theauth/auth` | Email + password with verification and reset |
