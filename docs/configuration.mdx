---
title: Configuration
description: Reference for every `createTheAuth()` option in `KavachConfig`. Covers database providers, agent settings, MCP OAuth, session config, and anomaly detection.
icon: gear
---


`createTheAuth()` accepts a `KavachConfig` object. The only required field is `database`. Everything else is optional and enables features incrementally.
