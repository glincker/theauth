...
`@theauth/electron` provides secure auth for Electron desktop apps with encrypted token storage, OAuth popup windows, and IPC bridge for renderer-to-main communication.

## Install

```bash
pnpm add @theauth/electron
```

## Setup (main process)

```ts
import { createKavachElectron } from '@theauth/electron';
...
```ts
import { useKavachElectron } from '@theauth/electron/renderer';
