...
W3C Decentralized Identifiers give agents a portable, cryptographic identity that works across services. Instead of an opaque token tied to one TheAuth instance, an agent gets a DID like `did:key:z6Mk...` backed by an Ed25519 keypair.
...
TheAuth supports two W3C DID methods:
...
<CardGroup cols={2}>
  ...
  <Card title="Agent identity" href="/agents" icon="robot">
    Bearer token identity and the agent lifecycle managed by TheAuth.
  </Card>
  <Card title="Standards alignment" href="/standards" icon="book-open">
    IETF draft claims TheAuth emits on agent JWTs.
  </Card>
</CardGroup>
