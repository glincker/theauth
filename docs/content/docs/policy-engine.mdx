...
Use the policy engine when you need to make authorization decisions that depend on org membership, relationship graphs, or delegation chains. The existing `authorize()` function still works and now delegates to `evaluate()` internally, so you only need to call the policy engine directly if you want access to richer decision metadata, cache statistics, or fine-grained invalidation control.
...
<TypeTable
  type={{
    'cache.maxEntries': {
      type: 'number',
      description: 'Maximum LRU entries. Evicts oldest entries when exceeded.',
      default: '10000',
    },
    'cache.ttlMs': {
      type: 'number',
      description: 'Time-to-live per cache entry, in milliseconds.',
      default: '60000',
    },
    'cache.enabled': {
      type: 'boolean',
      description: 'Set to false to disable the cache entirely. All evaluations go cold.',
      default: 'true',
    },
    combineStrategy: {
      type: '"deny-overrides" | "permit-overrides"',
      description: 'How to combine PERMIT and DENY results from multiple matching permissions.',
      default: '"deny-overrides"',
    },
    audit: {
      type: 'boolean',
      description: 'Whether to write audit rows. Disabling suppresses all audit writes.',
      default: 'true',
    },
    auditSampleRate: {
      type: 'number',
      description: 'Fraction of evaluations that write audit rows, from 0.0 to 1.0.',
      default: '1.0',
    },
  }}
/>
...
<Cards>
  <Card title="Permissions" description="Direct permissions, constraints, and templates." href="/docs/permissions" />
  <Card title="ReBAC" description="Relationship graph, hierarchy traversal, and tuple management." href="/docs/rebac" />
  <Card title="Delegation" description="Grant permission subsets between agents." href="/docs/delegation" />
  <Card title="Audit" description="How decisions are logged and surfaced in the dashboard." href="/docs/audit" />
</Cards>
